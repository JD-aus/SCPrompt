...
#AutoIt3Wrapper_Res_Fileversion=1.11.4.48
...
#EndRegion IsAdmin
#Region languages
Global $DebugLevel = "8", $DEBUGLOG = 0, $return, $AppINI = @ScriptDir & "\scprompt.ini", $TimeStampLoop[3], $INI_RemoteINI
$TimeStampLoop[1] = _NowCalc()
Global $V2M_EventDisplay
; New tranlation stuff - uses inireadsection
...
;Global $INI_LanguageSection = IniReadSection(@ScriptDir & "\v2m_lang.ini", "languages")
...
; old language stuff
;Global $V2M_GUI_Language = _Language()
;If StringLeft($V2M_GUI_Language, 1) = '"' Or StringLeft($V2M_GUI_Language, 1) = "'" Then
;	$V2M_GUI_Language = StringRegExpReplace($V2M_GUI_Language, '"', '')
;	$V2M_GUI_Language = StringRegExpReplace($V2M_GUI_Language, "'", '')
;EndIf
;Global $STG_Language_File = _Language_File($V2M_GUI_Language)
;Global $INI_Translation_Section = IniReadSection($STG_Language_File, "Lang_" & $V2M_GUI_Language)
;If IsArray($INI_Translation_Section) Then
;	$V2M_EventDisplay = V2M_EventLog("INI_" & $V2M_GUI_Language & "Section is a " & UBound($INI_Translation_Section, 0) & " dimensional array, with " & UBound($INI_Translation_Section) & " rows & " & UBound($INI_Translation_Section, 2) & " Coloumns.", $V2M_EventDisplay, "8")
;Else
;	$V2M_EventDisplay = V2M_EventLog("INI_" & $V2M_GUI_Language & "Section is NOT an array", $V2M_EventDisplay, "8")
;EndIf
...
If _RSC_INIReadSection_key($INI_CommonSection, "LANGUAGE") = "" Then
	Global $V2M_GUI_Language = _OSlangNames(@OSLang)
	$V2M_EventDisplay = V2M_EventLog("LANG - Automatic Language is found to be: '" & $V2M_GUI_Language & "'", $V2M_EventDisplay, "8")
Else
	Global $V2M_GUI_Language = "LANG_" & _RSC_INIReadSection_key($INI_CommonSection, "LANGUAGE")
...
Global $Write_Translation_file = 0

If FileExists(@ScriptDir & "\" & $V2M_GUI_Language & ".ini") Then
	Global $STG_Language_File = (@ScriptDir & "\" & $V2M_GUI_Language & ".ini")
	$V2M_EventDisplay = V2M_EventLog("LANG - $STG_Language_File = '" & $STG_Language_File & "'", $V2M_EventDisplay, "8")
	;ElseIf FileExists(@ScriptDir & "\TRANSLATE\" & $V2M_GUI_Language & ".ini") Then
	;	Global $STG_Language_File = (@ScriptDir & "\LANG\" & $V2M_GUI_Language & ".ini")
	;	$V2M_EventDisplay = V2M_EventLog("LANG - $STG_Language_File = '" & $STG_Language_File & "'", $V2M_EventDisplay, "8")
Else
	If StringInStr($CmdLineRaw, "-testing") Then
		...
		EndIf
	EndIf
	Global $STG_Language_File = (@ScriptDir & "\LANG_ENGLISH.ini")
	$V2M_EventDisplay = V2M_EventLog("LANG - $STG_Language_File = '" & $STG_Language_File & "'", $V2M_EventDisplay, "8")
EndIf
...
Global $INI_Translation_Section = IniReadSection($STG_Language_File, "LANG_" & $V2M_GUI_Language)
;Assign("INI_" & $V2M_GUI_Language & "Section", IniReadSection($STG_Language_File, $V2M_GUI_Language))
...
If IsArray($INI_Translation_Section) Then
	...
	;	If $Write_Translation_file = 1 Then
	;		IniWriteSection(@ScriptDir & "\Lang_" & $V2M_GUI_Language & ".ini", $V2M_GUI_Language, $INI_Translation_Section)
	;		$return = MsgBox(4, "Language file created", "Do you want to open it?", 10)
	;		If $return = 6 Then
	;			_StartSite(@ScriptDir & "\Lang_" & $V2M_GUI_Language & ".ini")
	;		EndIf
	;	EndIf
Else
...
EndIf
;Exit
;If $V2M_GUI_Language = 'Lang_' Or $V2M_GUI_Language = '' Then
;	$V2M_EventDisplay = V2M_EventLog("Language - Could not find a translation for: " & @OSLang, $V2M_EventDisplay, "1")
;Else
;	$V2M_EventDisplay = V2M_EventLog("Language - Determined Windows OSLang (" & @OSLang & ") to be: " & $V2M_GUI_Language, $V2M_EventDisplay, "2")
;EndIf
#EndRegion languages
...
Else
	...
	TrayTip(_RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_START_TITLE", "APP_START_TITLE"), _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_CONFIG_DOWNLOAD", "Downloading Configuration File"), 30)
	...
	ElseIf StringLeft($INI_RemoteINI, 7) = "http://" Or StringLeft($INI_RemoteINI, 8) = "https://" Then
		$s_GetVersFile = InetGet($INI_RemoteINI, @ScriptDir & "\scprompt_remote.ini", 1)
...
Global $INI_text1 = _RSC_INIReadSection_key($INI_GUISection, "TEXT1", "Select the Connection Name"), $INI_text2 = _RSC_INIReadSection_key($INI_GUISection, "TEXT2", "From the list"), $INI_text3 = _RSC_INIReadSection_key($INI_GUISection, "TEXT3", "SCPrompt 2011"), $INI_text4 = _RSC_INIReadSection_key($INI_GUISection, "TEXT4", "Provided by YTS")
Global $INI_text1BGColor = _RSC_INIReadSection_key($INI_ColourSection, "Text1BGColor", "0xefefef"), $INI_text2BGColor = _RSC_INIReadSection_key($INI_ColourSection, "Text2BGColor", "0xDedbe7")
Global $INI_bg_color1 = _RSC_INIReadSection_key($INI_ColourSection, "BGCOLOR", " "), $INI_bg_color2 = _RSC_INIReadSection_key($INI_ColourSection, "BGCOLOR2", " "), $INI_bg_color3 = _RSC_INIReadSection_key($INI_ColourSection, "BGCOLOR3", " "), $INI_ServiceMode = _RSC_INIReadSection_key($INI_CommonSection, "ServiceMode", "0")
Global $INI_Tel = _RSC_INIReadSection_key($INI_ColourSection, "TEL", ""), $INI_WebPage = _RSC_INIReadSection_key($INI_ColourSection, "WebPage", "")
Global $INI_colortext1 = _RSC_INIReadSection_key($INI_ColourSection, "COLORTEXT1", 0), $INI_colortext2 = _RSC_INIReadSection_key($INI_ColourSection, "COLORTEXT2", 0), $INI_colortext3 = _RSC_INIReadSection_key($INI_ColourSection, "COLORTEXT3", "0x848484"), $INI_colortext4 = _RSC_INIReadSection_key($INI_ColourSection, "COLORTEXT4", "0x848484")
Global $INI_SetPhoto = _RSC_INIReadSection_key($INI_ColourSection, "Set_photo", "1"), $INI_SetLogo = _RSC_INIReadSection_key($INI_ColourSection, "Set_logo", "1"), $INI_Set_webpage = _RSC_INIReadSection_key($INI_ColourSection, "Set_webpage", "0"), $INI_AutoReconnect = _RSC_INIReadSection_key($INI_CommonSection, "autoreconnect", "1"), $INI_UseSC_Server = _RSC_INIReadSection_key($INI_CommonSection, "UseSC_Server", "1")
Global $GUI, $test, $pngSrc, $hImage, $width, $height, $controlGui, $GUI_Label, $StopButton, $V2M_Status[15], $test, $V2M_LogoPic, $V2M_GUI_Width, $V2M_GUI_Height, $comboitem1, $comboitem2, $V2M_GUI, $tcpipitem, $tcpipitemLabel, $portitem, $portitemLabel, $idsitem, $V2M_Tray[9]
Global $V2M_NoGUI = 0, $curVal = "", $curtop = 10, $v2m_AdditionalHeight = 0, $v2m_AdditionalWidth = 0, $subitem = 1, $canitem = 1, $DisplayManual = 0, $V2M_VNC_UVNC = "winvnc.exe", $Use_UVNC_Server = 1
...
Global $YTS_BS_Checkbox1, $YTS_BS_Checkbox2, $YTS_BS_Checkbox3, $Beacon_Start_Button, $YTS_BS_Label, $gui2, $getpos, $Beacon_Exit_Button, $YTS_BS_Label
...
If $V2M_Exit <> 1 Then
	...
	Else
		...
		TrayTip(_RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_START_TITLE", "APP_START_TITLE"), _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_START_LINE1", "") & @CR & _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_START_LINE2", ""), 10)
	EndIf
EndIf
	...
	If _DateDiff('s', $TimeStampLoop[1], _NowCalc()) >= 1 Then
		...
		If $V2M_Status[11] Then ;uvncwanted
			...
			If $INI_AutoReconnect = 0 And $V2M_Status[12] = 1 And Not ProcessExists($V2M_VNC_UVNC) Then ;no reconnect, UVNC is meant to be started, but process not exist.
				...
				$V2M_Status[11] = 0
				TraySetState(_RSC_INIReadSection_key($INI_ColourSection, "TraySetStateShow", 9))
			ElseIf $INI_UseSC_Server = 1 And $INI_ServiceMode <> 1 And ProcessExists($V2M_VNC_UVNC) And ($V2M_Status[12] <> 1) Then
				V2M_EventLog($V2M_VNC_UVNC & " Application exists ... trying to kill", $V2M_EventDisplay, "3")
				_StopKnownVNCFlavour()
				Run(@ScriptDir & "\" & $V2M_VNC_UVNC & " -kill", @ScriptDir, @SW_HIDE)
			ElseIf $INI_UseSC_Server = 1 And $INI_ServiceMode <> 1 And ProcessExists("winvnc.exe") And ($V2M_Status[12] <> 1) Then
			...
			ElseIf Not ProcessExists($V2M_VNC_UVNC) And ($INI_UseSC_Server = 0 Or Not ProcessExists("winvnc.exe")) Then
				...
				$V2M_Status[12] = 1
				TrayTip(_RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_SC_START_TITLE", ""), _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_SC_START_LINE1", "") & @CR & _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_SC_START_LINE2", ""), 10)
			...
			Else
				...
				TraySetState(_RSC_INIReadSection_key($INI_ColourSection, "TraySetStateFlash", 4))
	...
	If _RSC_INIReadSection_key($INI_GUISection, "GUI_Type", "Radio") <> "Manual" Then
		...
		If $V2M_Status[2] <> "" Then
			...
			Else
				...
				$V2M_Status[11] = 1
				$V2M_Exit = 0
				TrayTip(_RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_SC_START_TITLE", ""), _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_SC_START_LINE1", "") & @CR & $V2M_Status[2] & @CR & _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_SC_START_LINE2", ""), 10)
			EndIf
		EndIf
	Else
...
Func OnAutoItExit()
	...
	If @Compiled And (@exitCode >= 3) Then
		...
		SetError(1)
	Else
		TrayTip(_RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_EXITING_TITLE", "APP_EXITING_TITLE"), _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_EXITING_LINE1", "") & @CR & _RSC_INIReadSection_key($INI_Translation_Section, "TRAYTIP_APP_EXITING_LINE2", ""), 30)
...
Func _VNCServiceMode($mode = "Start")
	...
	ElseIf $mode = "Stop" Then
		...
		ElseIf $UVNC_Installed_Path <> "" Then
			...
			Do
				...
				$V2M_EventDisplay = V2M_EventLog("SERVICE - Stopping Service, $LocalReturn = " & $LocalReturn & ", $Local_loop = " & $Local_loop, $V2M_EventDisplay, "8")
				Sleep(250)
				$Local_loop = $Local_loop + 1
	...
	Else ;start
		...
		If $UVNC_ExistingService <> 0 Then
			...
			$V2M_EventDisplay = V2M_EventLog("SERVICE - Stoped state = " & _SetServiceState("uvnc_service", "Stop"), $V2M_EventDisplay, "7")
