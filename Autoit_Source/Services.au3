...
Func _Service_Create($sComputerName, $sServiceName, $sDisplayName, $sBinaryPath, _
  ...
  Local $SERVICE_INTERROGATE = 0x00000080
  Local $hAdvapi32
  Local $hKernel32
  Local $arRet
  Local $hSC
...
;===============================================================================
; Function Name:   _Service_GetDescription()
; Description:     Return the Description of a Service
; Syntax:          _Service_GetDescription("Service Name"[, "Computer"])
...
Func _Service_GetDescription($sServiceName, $Computer = "localhost")
  _sServErrorHandlerRegister()
  ...
  Next
  Return $Rtn
EndFunc   ;<==> _Service_GetDescription()
...
;               $Dtl = StringSplit($Dtl & "Interact With DeskTop|Display Name|Error Control|Exec File Path|Started|Start Mode|Account", '|')
;                      For $I = 1 To $Var[0]
...
Func _Service_SetStartMode($sServiceName, $sMode, $Computer = "localhost")
   _sServErrorHandlerRegister()
   Local $Service = ObjGet("winmgmts:" _
    & "{impersonationLevel=impersonate}!\\" & $Computer & "\root\cimv2")
    Local $sQuery = "Select * from Win32_Service Where  name like '" & $sServiceName & "'"
...
; Description:   Sets StartType of a service on a computer
; Author:        MadBoy
; Parameters:    $sComputerName - name of the target computer. If empty, the local computer name is used
...
; Example(s):  MsgBox(0, "Test", _ServiceStartType("CarbonCopy32", "Disabled"))
;===============================================================================

Func _Service_StartType($sServiceName, $sStatus, $sComputerName = "localhost")
  Local $iValue
  Local $sServiceKeyPath = "HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Services\" & $sServiceName
  
  If $sStatus = "Auto" Then $iValue = "2"
  If $sStatus = "Manual" Then $iValue = "3"
  If $sStatus = "Disabled" Then $iValue = "4"
  
  If $sComputerName = "" Then
    If RegRead($sServiceKeyPath, "Start") <> "" Then
      If RegWrite($sServiceKeyPath, "Start", "REG_DWORD", $iValue) = 1 Then Return 1
    Else
      Return 0
    EndIf
  Else
    If RegRead("\\" & $sComputerName & "\" & $sServiceKeyPath, "Start") <> "" Then
      If RegWrite("\\" & $sComputerName & "\" & $sServiceKeyPath, "Start", "REG_DWORD", $iValue) = 1 Then Return 1
    Else
      Return 0
    EndIf
  EndIf
...
;===============================================================================
; Function Name:   _sServErrorHandlerDeRegister()
...
;===============================================================================
; Function Name:   _sServErrorHandlerRegister()
...
;					@ERROR		- 0 ($_ServiceStatus_Success) = No Error
;								- 1 ($_ServiceStatus_GeneralError) = General Error
